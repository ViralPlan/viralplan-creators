...
export async function processInput(companySelected, plan) {
  ...
  for (let i = 0; i < plan['content'].length; i++) {
    ...
    try {
      field = form.getTextField(
        `video_${videoNumber.toString().padStart(2, '0')}`
      );
      field.setText('VÍDEO ' + videoNumber.toString());
      field = form.getTextField(
        `guion_${videoNumber.toString().padStart(2, '0')}`
      );
      field.setText(video.guion);
      field = form.getTextField(
        `texto_${videoNumber.toString().padStart(2, '0')}`
      );
      field.setText(video.texto);
      field = form.getTextField(
        `audio_${videoNumber.toString().padStart(2, '0')}`
      );
      field.setText('                      ' + video.audio);
      field = form.getTextField(
        `descripcion_${videoNumber.toString().padStart(2, '0')}`
      );
      field.setText('                                    ' + video.descripcion);
      field = form.getTextField(
        `duracion_${videoNumber.toString().padStart(2, '0')}`
      );
      field.setText(video.duracion);
      field = form.getTextField(
        `extra_${videoNumber.toString().padStart(2, '0')}`
      );
      field.setText(video.extra);
      field = form.getTextField(
        `horario_${videoNumber.toString().padStart(2, '0')}`
      );
      field.setText(video.horario);
      field = form.getTextField(
        `lugar_${videoNumber.toString().padStart(2, '0')}`
      );
      field.setText(video.lugares);
      field = form.getTextField(
        `personas_${videoNumber.toString().padStart(2, '0')}`
      );
