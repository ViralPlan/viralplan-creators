...
class AiAgent {
  constructor(name, prompt, config) {
    this.name = name;
    this.promptAddition =
      ' You must ALWAYS return your corrections in valid JSON format, and translated to SPANISH in Spains dialect.';
          ...
          items: {
            ...
            properties: {
              ...
              description: {
                type: 'string',
                description:
                  'A long version of the video idea, between 70 and 100 words long',
              },
            },
      ...
      functions: [
        {
          name: 'writeIdeas',
          description:
            'Escribe o cambia ideas de video para tiktok con la información que se ha dado',
          parameters: schema,
        },
      ],
      function_call: {
...
export async function genIdeas(companyForm, num, model, apiKey) {
  ...
  const adminAgentPrompt = `You are the representative of ViralPlan. Your work is to create ideas
    ...
    creative and disruptive, not just informative about the brand. If you are given ideas, you should offer suggestions and corrections.
    Enfasis on FUN, CREATIVE AND DISRUPTIVE. ALL OF THEM MUST BE FUN AND DISRUPTIVE. What you return should be in SPANISH of Spain.`;
  ...
  const expertAgentPrompt = `You are an expert in Tiktok trends and marketing. Your job is to talk
    ...
    return the same number of ideas the other agent gave you.
    Enfasis on FUN, CREATIVE AND DISRUPTIVE. ALL OF THEM MUST BE FUN AND DISRUPTIVE. `;
  ...
  const tiktokerAgentPrompt = `You are a big Tiktok star, and you know what makes a video go viral.
    ...
    Enfasis on FUN, CREATIVE AND DISRUPTIVE. ALL OF THEM MUST BE FUN AND DISRUPTIVE. 
    Everything you do, return just VALID JSON STRING. If they are not in spanish FROM SPAIN, translate them.`;
  ...
  const tiktokerAgent = new AiAgent(
    'tiktoker',
    tiktokerAgentPrompt,
    llm_config_1
  );
